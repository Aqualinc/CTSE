...
DownloadClifloData<-function(BaseDirectory="C:\\Egnyte\\Shared\\Canterbury GeoRURAL Pro\\Projects\\Jono\\Climate Data\\Cliflo downloading\\",Agentfile="agents.csv",parameter="rainfall")
  #  DownloadClifloData<-function(BaseDirectory="\\\\some\\directory\\where\\the\\automation\\stuff\\is")
{
  ...
  defaultimacro<-readLines(paste0(BaseDirectory,"ClifloDownloader9amtemp.iim"),-1)
  ...
  for (AgentNo in agents) {
    ...
    defaultimacro[17] <- AgentLine
    defaultimacro[36] <- OutputLine
    ...
    Sys.sleep(20)  #this is to ensure the downloading has ocurred before proceeding further

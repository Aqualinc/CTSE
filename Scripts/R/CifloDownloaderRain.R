...
DownloadClifloData<-function(BaseDirectory="C:\\Users\\j.dixon\\Desktop\\Base directory\\",Agentfile="agents.csv",parameter="rainfall")
  #  DownloadClifloData<-function(BaseDirectory="\\\\some\\directory\\where\\the\\automation\\stuff\\is")
{
  
  #remember to change this imacro if appropriate:
  
  defaultimacro<-readLines(paste0(BaseDirectory,"ClifloDownloaderRain.iim"),-1)
  ...
  Sys.sleep(3)  #this is to ensure Firefox has completed opening before starting the next step
  ...
  for (AgentNo in agents) {
    ...
    defaultimacro[15] <- AgentLine
    defaultimacro[34] <- OutputLine
    writeLines(defaultimacro,paste0(BaseDirectory,"NewClifloDownloader.iim"))
    CopyShellText <- paste0('copy /Y "',BaseDirectory,'NewClifloDownloader.iim" C:\\Users\\j.dixon\\Documents\\iMacros\\Macros\\NewClifloDownloader.iim')
    shell(CopyShellText)
    
    FirefoxCommand2 <- paste0('start /B /wait "" "',FirefoxPath, '" imacros://run/?m=NewClifloDownloader.iim')
    shell(FirefoxCommand2)
    #have made the following longer:
    Sys.sleep(25)  #this is to ensure the downloading has ocurred before proceeding further

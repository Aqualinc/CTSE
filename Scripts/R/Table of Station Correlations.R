############################################
# Jonathan Dixon  
#writes a table of all the extended data correlations in a folder
...
for (file in fileList){
  ...
  #correlation is in the 4th column:
  correlationData <- readData[,4]
  
  
  finalTable[,columnPosition] <- correlationData
...
outFileName <- file.path(WriteToDirectory,"Correlation extended summary.csv",fsep = "\\") 

...
FromDirectory <- "C:\\Users\\j.dixon\\Desktop\\Deal with duplicates\\Min T\\Raw"

#specify the directory to write the formatted files to
ToDirectory <- "C:\\Users\\j.dixon\\Desktop\\Deal with duplicates\\Min T\\Converted"

#directory if non-existent data
FaultyDirectory <- "C:\\Users\\j.dixon\\Desktop\\Deal with duplicates\\Min T\\"
...
for (file in fileList){
  ...
  if (length(readLines(fileOfInterest)) > 0){
    ...
    if (ncol(data)>2){ #if 3 or more columns present
      #eliminate those rows with dashes in 7th column, ADJUST THE COLUMN IF NEEDED!
      data <- data[which(!data[,7]=="-"),]
      
      #then prepare the file
      outFileName <- file.path(ToDirectory,file,fsep = "\\")
      ColumnsOfInterest <- c(3,7)
